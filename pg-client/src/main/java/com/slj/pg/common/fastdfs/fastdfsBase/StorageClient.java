...
import com.slj.pg.common.fastdfs.common.Base64;
...
public class StorageClient {
    ...
    protected ClientGlobal clientGlobal;
     ...
     */
    public StorageClient(TrackerServer trackerServer, StorageServer storageServer, ClientGlobal clientGlobal) {
     ...
     * return null if fail
     */
    public String[] upload_file(byte cmd, String group_name, File file, String file_ext_name,
                                NameValuePair[] meta_list) throws IOException, MyException {
        ...
        try {
            return this.do_upload_file(cmd, group_name, null, null, file_ext_name,
                    file.length(), new UploadStream(fis, file.length()), meta_list);
        } finally {
            fis.close();
        }
    }
     ...
     * return null if fail
     */
    public String[] upload_file(byte cmd, String group_name, InputStream inputStream, long fileLength, String file_ext_name,
                                NameValuePair[] meta_list) throws IOException, MyException {
...
//        FileInputStream fis = new FileInputStream(file);
        ...
        try {
            return this.do_upload_file(cmd, group_name, null, null, file_ext_name,
                    fileLength, new UploadStream(inputStream, fileLength), meta_list);
        } finally {
            inputStream.close();
        }
    }
     ...
     * @return 0 for success, != 0 for error (error no)
     */
    public int append_file(String group_name, String appender_filename, File file) throws IOException, MyException {
//        File f = new File(local_filename);
        FileInputStream fis = new FileInputStream(file);

        try {
            return this.do_append_file(group_name, appender_filename, file.length(), new UploadStream(fis, file.length()));
        } finally {
            fis.close();
        }
    }
    ...
    protected String[] do_upload_file(byte cmd, String group_name, String master_filename,
        ...
        try {
            ...
            if (file_ext_name != null && file_ext_name.length() > 0) {
                byte[] bs = file_ext_name.getBytes(clientGlobal.getG_charset());
            ...
            if (bUploadSlave) {
                masterFilenameBytes = master_filename.getBytes(clientGlobal.getG_charset());
            ...
            if (bUploadSlave) {
                ...
                byte[] bs = prefix_name.getBytes(clientGlobal.getG_charset());
    ...
    protected int do_append_file(String group_name, String appender_filename,
        ...
        try {
            ...
            appenderFilenameBytes = appender_filename.getBytes(clientGlobal.getG_charset());
     ...
     */
    public InputStream download_file(String group_name, String remote_filename) throws IOException, MyException {
        final long file_offset = 0;
        final long download_bytes = 0;

        return this.download_fileInputStream(group_name, remote_filename, file_offset, download_bytes);
    }
     ...
     * @return file content/buff, return null if fail
     */
    public InputStream download_fileInputStream(String group_name, String remote_filename, long file_offset, long download_bytes) throws IOException, MyException {
        boolean bNewConnection = this.newReadableStorageConnection(group_name, remote_filename);
        Socket storageSocket = this.storageServer.getSocket();
        ...
        try {
            ...
            pkgInfo = ProtoCommon.recvPackage(in,
                    ProtoCommon.STORAGE_PROTO_CMD_RESP, -1);
            ...
            inputStream = new ByteArrayInputStream(pkgInfo.body);
            return inputStream;
        } catch (IOException ex) {
            if (!bNewConnection) {
                try {
    ...
    public NameValuePair[] get_metadata(String group_name, String remote_filename) throws IOException, MyException {
        ...
        try {
            ...
            return ProtoCommon.split_metadata(new String(pkgInfo.body, clientGlobal.getG_charset()));
        } catch (IOException ex) {
            if (!bNewConnection) {
                try {
                    this.storageServer.close();
    ...
    public int set_metadata(String group_name, String remote_filename,
        ...
        try {
            ...
            if (meta_list == null) {
                ...
                meta_buff = ProtoCommon.pack_metadata(meta_list).getBytes(clientGlobal.getG_charset());
            }

            filenameBytes = remote_filename.getBytes(clientGlobal.getG_charset());
            sizeBytes = new byte[2 * ProtoCommon.FDFS_PROTO_PKG_LEN_SIZE];
            ...
            bs = group_name.getBytes(clientGlobal.getG_charset());
    ...
    protected boolean newWritableStorageConnection(String group_name) throws IOException, MyException {
        if (this.storageServer != null) {
            ...
            TrackerClient tracker = new TrackerClient(clientGlobal);
