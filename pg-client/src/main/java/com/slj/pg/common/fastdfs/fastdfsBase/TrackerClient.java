...
public class TrackerClient {
     ...
     */
    public TrackerClient(ClientGlobal clientGlobal) {
        this.clientGlobal = clientGlobal;
        this.tracker_group = clientGlobal.getG_tracker_group();
    }
    ...
    public StorageServer getStoreStorage(TrackerServer trackerServer, String groupName) throws IOException {
        ...
        try {
            ...
            if (groupName != null && groupName.length() > 0) {
                ...
                bs = groupName.getBytes(clientGlobal.getG_charset());
            ...
            return new StorageServer(ip_addr, port, store_path,clientGlobal);
    ...
    public StorageServer[] getStoreStorages(TrackerServer trackerServer, String groupName) throws IOException {
        ...
        try {
            ...
            for (int i = 0; i < serverCount; i++) {
                ...
                results[i] = new StorageServer(ip_addr, port, store_path,clientGlobal);
            }
    ...
    public StorageServer getFetchStorage(TrackerServer trackerServer,
        ...
        if (servers == null) {
            ...
            return new StorageServer(servers[0].getIpAddr(), servers[0].getPort(), 0,clientGlobal);
        }
    }
    ...
    protected ServerInfo[] getStorages(TrackerServer trackerServer,
        ...
        try {
            bs = groupName.getBytes(clientGlobal.getG_charset());
            bGroupName = new byte[ProtoCommon.FDFS_GROUP_NAME_MAX_LEN];
            bFileName = filename.getBytes(clientGlobal.getG_charset());
    ...
    private boolean deleteStorage(TrackerServer trackerServer,
        ...
        int ipAddrLen;
        byte[] bIpAddr = storageIpAddr.getBytes(clientGlobal.getG_charset());
