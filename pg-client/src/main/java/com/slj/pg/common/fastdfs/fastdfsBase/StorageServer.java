...
public class StorageServer extends TrackerServer {
     ...
     */
    public StorageServer(String ip_addr, int port, int store_path, ClientGlobal clientGlobal) throws IOException {
        super(clientGlobal.getSocket(ip_addr, port), new InetSocketAddress(ip_addr, port), clientGlobal.g_network_timeout, clientGlobal.g_connect_timeout);
     ...
     * @param store_path the store path index on the storage server
     */
    public StorageServer(String ip_addr, int port, byte store_path, ClientGlobal clientGlobal) throws IOException {
        super(clientGlobal.getSocket(ip_addr, port), new InetSocketAddress(ip_addr, port), clientGlobal.g_network_timeout, clientGlobal.g_connect_timeout);
        if (store_path < 0) {
